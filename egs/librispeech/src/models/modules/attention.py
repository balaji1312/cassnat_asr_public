...
def attention(query, key, value, mask=None, dropout=None):
    ...
    if mask is not None:
        #min_value obtained by querying machine for float limit
...
class MultiHeadedAttention(nn.Module):
    ...
    def __init__(self, h, d_model, dropout=0.1):
        ...
        self.d_k = d_model // h
        self.h = h
        #create 4 linear layers; 3 for generating k,q,v; one for final output
    ...
    def forward(self, query, key, value, mask=None, pos_embed=None):
        ...
        """
        if mask is not None:
            # ///OG Same mask applied to all h heads.
            mask = mask.unsqueeze(1)
        nbatches = query.size(0)
        
        #generate k,q,v applying linear layers, and then reshape output
        # ///OG 1) Do all the linear projections in batch from d_model => h x d_k 
        query, key, value = [l(x).view(nbatches, -1, self.h, self.d_k).transpose(1, 2)
             for l, x in zip(self.linears, (query, key, value))]
        
        #///OG 2) Apply attention on all the projected vectors in batch. 
        x, self.attn = attention(query, key, value, mask=mask, dropout=self.dropout)
        
        #///OG 3) "Concat" using a view and apply a final linear. 
...
class RelMultiHeadedAttention(nn.Module):
    ...
    def forward(self, query, key, value, mask=None, pos_embed=None):
        ...
        # 1)///OG  Do all the linear projections in batch from d_model => h x d_k 
        query, key, value = [l(x).view(nbatches, -1, self.h, self.d_k)
             for l, x in zip(self.linears, (query, key, value))]
        t_q = query.size(1)
        #pass pos emb through nn and reshape output
        pos_embed = self.linear_pos(pos_embed).view(-1, self.h, self.d_k).unsqueeze(0)  #(nb, 2*t_q-1, h, d_k)
        
        # 2) ///OG Apply attention on all the projected vectors in batch. 
        
        #reimplement attention for relative emb following Transformer XL (create scores with emb, key)
        ...
        #pad scores to facilitate adding and then normalise with sqrt(d_k)
