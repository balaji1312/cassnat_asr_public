...
class SingleSet(object):
    def __init__(self, data_path, rank):
        #similar to singleset in speech_loader, without the text labels
    ...
    def _load_label(self, lab_path, is_text=True):
        #not used for ssl as labels are not present
        label_dict = dict()
...
class SSLDataset(Dataset):
    def __init__(self, data_paths, args):
        #similar to init for speechdataset in speechloader, without context, skip and vocab
        self.rank = args.rank
    ...
    def __getitem__(self, idx):
        #similar to speechdataset, without padding or context (no labels also extracted); also ensures specaug is not used
        stream_idx = -1
    ...
    def specaug_mask(self, feat):
        ...
        labels = (feat - x) #make unmasked regions 0 for loss func
        return x, labels
    ...
    def random_mask(self, fbank):
        ...
        for i, frame in enumerate(fbank):
            prob = random.random()
            #15% of time we modify input token to either mask, random or current (as done in bert)
            if prob < 0.15:
                prob /= 0.15
...
class DynamicDataset(Dataset):
    #most functions are a combination of speech_loader dynamic dataset and ssl dataset
    ...
    def __getitem__(self, idx):
        #similar to getitem in dynamic dataset in speechloader, except we derive labels from ssl type, and perform padding 
        #and skip+context on the labels as well
        ...
        for i in range(len(batch)):
            ...
            #based on type of ssl, we obtain the labels before performing specaug
...
class MyDataLoader(DataLoader):
    ...
    def collate_fn(self, batch):
        #similar to collate_fn in supervise_loader, except we already have total label length as shape of label, 
        # and divide to obtain size
...
class SSLDataLoader(MyDataLoader):
    #similar to ssl dataloader in speech_loader
