...
def ctc_greedy_wer(ctc_out, label, feat_size, pad=0):
    ...
    pred = torch.max(ctc_out, dim=-1)[1].cpu().numpy() #ctc_out: bs x length x vocab
    bs, length = pred.shape
    for i in range(bs):
        p_seq = []
        h_seq = []
        #append pred out and label to obtian contiguous seq
        ...
        #compute dist between two seq (levenshtein dist from editdistance)
