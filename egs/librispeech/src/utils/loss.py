...
class LabelSmoothing(nn.Module):
    ...
    def forward(self, x, target):
        ...
        taget: bU
        """
        assert x.size(1) == self.size #vocab size must be same as preds for each word
        with torch.no_grad():
            true_dist = x.clone()
            ...
            true_dist.scatter_(1, target.unsqueeze(1), self.confidence) #fill values with confidence
            self.true_dist = true_dist
        return self.criterion(x, true_dist).masked_fill(mask.unsqueeze(1)==0, 0).sum() / tokens #compute kldiv loss
    ...
    def MWER(self, att_out, ctc_target, wer_weight):
        ...
        #///OG batch_size, sample_num = norm_prob.size()
        ...
        wer_loss = (att_prob * wer_weight) #///OG * norm_prob
...
class KLDivLoss(nn.Module):
    ...
    def forward(self, x, at_prob, target):
        ...
        taget: bU x class
        """
        #similar to forward in labelsmoothing
