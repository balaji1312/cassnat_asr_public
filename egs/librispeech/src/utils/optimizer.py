...
class BaseOpt(object):
    #baseoptimiser class: defines diff learning rates and how to transition between them
    ...
    def load_state_dict(self, state_dict, rank, use_cuda):
        """Load state_dict."""
        #from state_dict sets optimiser params
...
class MulBaseOpt(object):
    def __init__(self, optimizer, freeze_steps=None):
        #similar to base class, but we index self.optimiser instead of creating more instances
        self.optimizer = optimizer
        self._step = 0
    ...
    def step(self):
        #similar to base class, but we also set future rates to 0 if they exceed freeze_steps
        self._step += 1
        rate = self.rate()
...
class NoamOpt(BaseOpt):
    ...
    def __init__(self, optimizer, model_size=512, factor=5.0, warmup_steps=25000, total_steps=40000, warmup_type="noam_warmup"):
        #similar to base class, with optimiser passed to sup init
        self.factor = factor
    ...
    def rate(self, step = None):
        ...
        if self.warmup_type == "noam_warmup":
            ...
            if self.warmup_type == "custom_exp": #typically used for transformer
                return c * step ** (-0.5)
            elif self.warmup_type == "custom_linear": #can get 0 learning rate at end
...
class MulNoamOpt(MulBaseOpt):
    ...
    def __init__(self, optimizer, model_size, factors, warmup_steps, freeze_steps, total_steps=40000, warmup_type="noam_warmup"):
        #similat to noamopt combined with mulopt

        #///OG factors: list   warmup_steps: list 
        self.optimizer = optimizer
...
class LRMulStepScheduler(BaseOpt):
    #schedule system based on few args
    ...
    def rate(self, step = None):
        #simplified schedule system for lr
        if step is None:
            step = self._step
        if step <= self.s_warm:
...
def get_optim(optim_type, model, args):
    #defines adam opt and calls one of the above schedulers
...
def get_mul_optim(optim_type, update_params_group, args):
    #defines adam opt and calls mulnoam scheduler
    eps = args.eps if hasattr(args, 'eps') else 1e-9
    updated_params = []
