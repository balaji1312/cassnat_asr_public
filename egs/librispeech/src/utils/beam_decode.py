...
def ctc_beam_decode(model, src, src_mask, src_size, vocab, args, lm_model=None):
    # ///OG decoding with lm is not applicable temporarily
    bs = src.size(0)
    ...
    #get largest probs for each input
    ...
    for t in range(ctc_out.size(1)):
        if lm_model is not None:
            ys, ys_size = [], []
            for b in range(bs):
                if t > src_size[b].item() or torch.exp(ctc_out[b, t, blank]).item() > 0.95: #blank prob higher than 0.95
                    continue
                ...
                for seq in batch_top_seqs[b]:
                    ...
                    ys.append(ys_fill) #copy prev entries

            if len(ys) == 0: #implies batch is empty
                continue
            else:
                max_ys += 1
            ...
            #using mask obtain largest prob from lm model
