...
from models import make_hubert_model
...
class HubertTask(BaseTask):
    def __init__(self, mode, args):
        super(HubertTask, self).__init__(args)
    ...
    def set_model(self, args):
        #check if arg dimensions match, and then create model

        self.model = make_hubert_model(args)
    ...
    def run_one_epoch(self, epoch, args, is_train=True):
        ...
        for i, data in enumerate(dataloader):
            ...
            ctc_errs, all_tokens = ctc_greedy_wer(ctc_out, tgt_label.cpu().numpy(), audio_sizes.cpu().numpy(), args.padding_idx)
    ...
    def decode(self, args):
        ...
        with torch.no_grad():
            ...
            for i, data in enumerate(self.test_loader):
                utt_list, audios, labels, audio_sizes, label_sizes = data
                src, src_mask = audios, (audios[:,0] != args.padding_idx).unsqueeze(1)
                ...
                if args.decode_type == 'ctc_only':
                    recog_results = ctc_beam_decode(self.model, src, src_mask, audio_sizes, self.vocab, args, self.lm_model)
                
                elif args.decode_type == 'ctc_att':
                    batch_top_seqs = ctc_beam_decode(self.model, src, src_mask, audio_sizes, self.vocab, args, self.lm_model)
                    recog_results, args = self.model.beam_decode(src, src_mask, audio_sizes, self.vocab, args, self.lm_model, batch_top_seqs, labels=labels, label_sizes=label_sizes)
                
                else:
                    recog_results, args = self.model.beam_decode(src, src_mask, audio_sizes, self.vocab, args, self.lm_model, labels=labels, label_sizes=label_sizes)
