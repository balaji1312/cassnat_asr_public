...
from models import make_lmnat3_model
...
class LMNAT3Task(BaseTask):
    #hybrid of cassnat and lmnat functions
    def __init__(self, mode, args):
        super(LMNAT3Task, self).__init__(args)
        ...
        if mode == "test":
            ...
            args.label_smooth = 0
            self.set_model(args)
            self.model.remove_unused_modules()
    ...
    def set_model(self, args):
        ...
        self.model = make_lmnat3_model(args.input_size, args)
    ...
    def load_model(self, args):
        ...
        if os.path.exists(last_checkpoint):
            ...
            self.load_checkpoint(last_checkpoint, args.rank, args.use_gpu, args.freeze_sad)
        else:
            self.load_pretrained_model(args)
            self.model.remove_unused_modules()
            self.set_optimizer(args)

    def load_checkpoint(self, checkpoint, rank, use_cuda, freeze_sad):
        if rank == 0:
        ...
        if freeze_sad:
            for name, param in self.model.sad.named_parameters():
    ...
    def load_pretrained_model(self, args):
        ...
        if args.init_sad and args.text_encoder_path:
            if args.rank == 0:
            ...
            if args.sad_type == "lm":
                ...
                self.model.sad.transformer = load_tf_weights_in_gpt2(self.model.sad.transformer, gpt2_checkpoint)
                ...
                self.model.sad = load_tf_weights_in_bert(self.model.sad, bert_checkpoint)
    ...
    def set_optimizer(self, args):
        ...
        encoder_updated = list(func(self.model.ctc_generator))
        if args.interctc_alpha > 0:
            encoder_updated += list(func(self.model.interctc_generator))

        decoder_updated = list(func(self.model.taee)) + list(func(self.model.mad)) \
                            + list(func(self.model.att_generator)) + list(func(self.model.pre_dim_map)) \
                            + list(func(self.model.post_dim_map))

        if self.model.sos_embed is not None:
            decoder_updated.append(self.model.sos_embed)
        if self.model.mask_embed is not None:
            decoder_updated.append(self.model.mask_embed)

        if args.plm_alpha > 0:
            decoder_updated += list(func(self.model.plm_generator))

        pretrained_decoders = list(func(self.model.sad))
        update_param_groups = [pretrained_encoders, encoder_updated, pretrained_decoders, decoder_updated]
    ...
    def run_one_epoch(self, epoch, args, is_train=True):
        ...
        plm_losses = util.AverageMeter('PlmLoss', ":.4e")
        ...
        progress = util.ProgressMeter(num_updates, batch_time, losses, ctc_losses, att_losses, plm_losses, ctc_wers, \
        ...
        for i, data in enumerate(dataloader):
            ...
            ctc_out, att_out, loss, ctc_loss, att_loss, plm_loss = self.model(src, src_mask, feat_sizes, tgt_label, label_sizes, args)
            ...
            plm_losses.update(plm_loss.item(), 1)
        ...
        with torch.no_grad():
            ...
            for i, data in enumerate(self.test_loader):
                ...
                if args.decode_type == 'ctc_only':
                    ...
                    recog_results, args = self.model.beam_decode(src, src_mask, feat_sizes, self.vocab, args, self.lm_model, labels, label_sizes)
