...
# 2022 Ruchao Fan
# we replace SAD with pretrained language models (BERT, GPT2) and
# apply corresponding lm loss to it
...
from models.blocks import ConMAD, TrfMAD, ConAcExtra, TrfAcExtra
...
def make_model(input_size, args):
    ...
    plm_gen = Generator(args.d_model, args.vocab_size, add_norm=False) if args.plm_alpha > 0 else None
    pe = create_pe(args.d_model)

    # set up pretrained language models as SAD
    if args.sad_type == "lm":        
    ...
    model = LMNAT3(
                ...
                encoder, Extra, text_encoder, Mad, c(generator), c(generator),
                pe, interctc_gen, plm_gen, args)
...
class LMNAT3(nn.Module):
    def __init__(self, src_embed, encoder, taee, text_encoder, mad, ctc_gen, att_gen, pe, interctc_gen, plm_gen, args):
        super(LMNAT3, self).__init__()
        self.src_embed = src_embed
        self.encoder = encoder
        self.taee = taee
        self.pre_dim_map = nn.Linear(args.d_model, text_encoder.dim)
        self.sad = text_encoder
        self.post_dim_map = nn.Linear(text_encoder.dim, args.d_model)
        self.mad = mad
        self.ctc_generator = ctc_gen
        self.att_generator = att_gen
        self.pe = pe
        if args.use_mask_embed:
            self.mask_embed = nn.Parameter(torch.FloatTensor(text_encoder.dim))
        else:
            self.mask_embed = None

        if args.sad_type in ["lm", "gpt2"]:
            self.sos_embed = nn.Parameter(torch.FloatTensor(text_encoder.dim))
        else:
            self.sos_embed = None
        ...
        if plm_gen is not None:
            self.plm_generator = plm_gen
            self.plm_loss = LabelSmoothing(args.vocab_size, args.padding_idx, 0) #args.label_smooth)
    ...
    def forward(self, src, src_mask, src_size, tgt_label, label_sizes, args):
        ...
        if args.pos_type == "relative":
            mapped_acoustic_embed = self.pre_dim_map(token_acoustic_embed[0])
            pos_embed = token_acoustic_embed[1]
        else:
            mapped_acoustic_embed = self.pre_dim_map(token_acoustic_embed)

        if args.apply_mask:
            B, T, _ = mapped_acoustic_embed.size()
            mask_indices = compute_mask_indices((B, T), ~tgt_mask_bidi.bool(), args.mask_prob, args.mask_length, 
                                args.mask_selection, args.mask_other, min_masks=2, no_overlap=args.no_mask_overlap, 
                                min_space=args.mask_min_space, require_same_masks=args.require_same_masks, 
                                mask_dropout=args.mask_dropout)
            mask_indices = torch.from_numpy(mask_indices).to(mapped_acoustic_embed.device)
            if args.use_mask_embed:
                mapped_acoustic_embed[mask_indices] = self.mask_embed
            else:
                for b in range(mapped_acoustic_embed.size(0)):
                    mapped_acoustic_embed[b][mask_indices[b]] = mapped_acoustic_embed[b].mean(0)
        else:
            mask_indices = None

        if args.sad_type in ["lm", "gpt2"]:
            sos_embed = self.sos_embed.unsqueeze(0).unsqueeze(1).repeat(mapped_acoustic_embed.size(0), 1, 1)
            mapped_acoustic_embed = torch.cat([sos_embed, mapped_acoustic_embed[:,:-1,:]], dim=1)
            plm_target = tgt_label

        if args.sad_type == "lm":    
            tgt_mask = tgt_mask_bidi & self.subsequent_mask(ymax).type_as(tgt_mask_bidi) # uni-direc
            pred_embed, _ = self.sad.forward_backbone(mapped_acoustic_embed, tgt_mask)

        elif args.sad_type == "gpt2":
            pred_embed, _ = self.sad.forward_backbone(mapped_acoustic_embed)
        
        elif args.sad_type == "bert":
            pred_embed, _ = self.sad.forward_backbone(mapped_acoustic_embed, attention_mask=tgt_mask_bidi.squeeze(1))
            if args.plm_alpha > 0:
                assert mask_indices is not None
                plm_target = tgt_label[mask_indices]
    
        pred_embed = self.post_dim_map(pred_embed)

        if args.plm_alpha > 0:
            plm_input = pred_embed[mask_indices] if mask_indices is not None else pred_embed
            plm_out = self.plm_generator(plm_input)
        ...
        if args.pos_type == "relative":
            pred_embed = (pred_embed, pos_embed)

        dec_h = self.mad(pred_embed, enc_h, x_mask, tgt_mask_bidi)
        ...
        if args.plm_alpha > 0:
            plm_loss = self.plm_loss(plm_out.view(-1, plm_out.size(-1)), plm_target.view(-1))
            loss += args.plm_alpha * plm_loss
        else:
            plm_loss = torch.Tensor([0])

        return ctc_out, att_out, loss, ctc_loss, att_loss, plm_loss
    ...
    def remove_unused_modules(self):
        self.sad.remove_unused_module_aggressive()
    
    def beam_decode(self, src, x_mask, src_size, vocab, args, lm_model, ctc_top_seqs=None, labels=None, label_sizes=None):
        ...
        if args.sad_type == "lm":    
            ...
            pred_embed, _ = self.sad.forward_backbone(mapped_acoustic_embed, attention_mask=tgt_mask1.squeeze(1))
        ...
        dec_h = self.mad(pred_embed, enc_h, src_mask, tgt_mask1)
        att_out = self.att_generator(dec_h)
