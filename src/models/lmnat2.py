...
def make_model(input_size, args):
    ...
    if args.use_conv_dec:
        ...
        Sad = ConMAD(args.d_model, c(dec_ff), c(dec_self_attn), c(dec_conv_module), c(dec_src_attn), c(dec_ff), args.dropout, args.N_self_dec, args.pos_type, args.share_ff)
        ...
        Sad = TrfMAD(args.d_model, c(dec_attn), c(dec_attn), c(dec_ff), args.dropout, args.N_self_dec)
...
class LMNAT2(nn.Module):
    ...
    def forward(self, src, src_mask, src_size, tgt_label, label_sizes, tokenizer, text_encoder_tokenizer, args):
        ...
        # 4. obtain text_embed from ctc_output
        ...
        # 5. SAD with text_encoder output
        pred_embed = self.sad(token_acoustic_embed, text_embed, text_mask, tgt_mask_bidi, args.selfce_alpha, args.interce_layer, False)
        if args.selfce_alpha > 0:
            interce_out = self.interce_generator(pred_embed[-1])
            pred_embed = pred_embed[:-1]

        # 6.. decoder, output units generation
        if args.use_unimask:
        ...
        # 7. decoder with acoustic and text ouputs
        if args.src_trigger:
            x_mask = trigger_mask
        dec_h = self.mad(pred_embed, enc_h, x_mask, tgt_mask, args.mixce_alpha, args.interce_layer)
    ...
    def beam_decode(self, src, x_mask, src_size, args, lm_model, tokenizer, text_encoder_tokenizer, ctc_top_seqs=None, labels=None, label_sizes=None):
        ...
        # 4. obtain text_embed from ctc_output, we use greedy results as text encoder input without ESA
        text_inputs = []
        ...
        pred_embed = self.sad(token_acoustic_embed, text_embed, text_mask, tgt_mask1, is_last=False)

        # 5. decoder, output units generation
        if args.use_unimask:
            sos_embed = torch.zeros(pred_embed.size(0), 1, pred_embed.size(2)).type_as(pred_embed)
            pred_embed = torch.cat([sos_embed, pred_embed[:,:-1,:]], dim=1)
        ...
        dec_h = self.mad(pred_embed, enc_h, src_mask, tgt_mask)
        att_out = self.att_generator(dec_h)
