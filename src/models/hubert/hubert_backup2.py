...
from workdir.cassnat_asr.src.models.hubert.hubert_blocks import ConvFeatureExtractionModel

def make_model(input_size, args):
    """Creates model based on passed in input size (no. of features for ASR) and arguments defined in bash script"""

    #define copy function to pass into other functions (standard python ML procedure)
    c = copy.deepcopy

    # ///OG: we do not make a comparison with relative and absolute in CASS_NAT
    ...
    post_extract_proj = (
            nn.Linear(embed, args.encoder_embed_dim)
            if embed != args.encoder_embed_dim
            else None
        )

    final_dim = args.final_dim
    
    dropout_input = nn.Dropout(args.dropout_input)
    dropout_features = nn.Dropout(args.dropout_features)
    ...
    layer_norm = LayerNorm(embed)

    final_proj = nn.Linear(args.encoder_embed_dim, final_dim)

    num_classes = [len(d) for d in args.dictionaries]
    label_embs_concat = nn.Parameter(
        torch.FloatTensor(sum(num_classes), final_dim)
    )
    nn.init.uniform_(label_embs_concat)

    #Define model layers for decoder and token extractor; use_conv_dec is set in cassnat_train.yaml
...
class CassNAT(nn.Module):
    ...
    def viterbi_align(self, ctc_out, src_mask, src_size, ys, ylens, blank, sample_topk):
        ...
        same_transition = (path[:, :-2] == path[:, 2:]) #///OG including blank-2-blank
