...
def make_model(args):
    ...
    # # load config
    # config_file = os.path.join(config_path, 'hparams.json')
    # model_path = os.path.join(args.exp_dir, args.resume_model)
    # _, config, __ = load_model_ensemble_and_task([model_path])

    # hfile = open(config_path, 'rb')
      
    # cfg, task= pickle.load(hfile)                     
    # hfile.close()

    # hubert_config = convert_namespace_to_omegaconf(cfg)

    # task_config = tasks.setup_task(hubert_config.task)

    # task_config.load_state_dict(task)


    # hubert_config = HubertConfig()
    # hubert_config.load_from_json(config_file)
    args.d_model = args.encoder_embed_dim

    # task_config.cfg.fine_tuning = True
    ...
    model = HubertNAT(hub_base, Extra, Sad, Mad, c(generator), c(generator), pe, interctc_gen, interce_gen, args)
...
class HubertNAT(nn.Module):
    def __init__(self, hub_base, acembed_extractor, embed_mapper, decoder, ctc_gen, att_gen, pe, interctc_gen, interce_gen, args):
    ...
    def forward(self, src, src_mask, src_size, tgt_label, label_sizes, args):
        ...
        pe = self.pe.type_as(x).unsqueeze(0).repeat(bs, 1, 1)[:,:ymax,:]
